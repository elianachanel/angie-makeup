...
export function ReservationsTable({ reservations, serviceTitles, loading }: Props) {
  ...
  const filtered = useMemo(
    () => filterReservations(reservations, query, statusFilter, dateFilter || undefined),
    [reservations, query, statusFilter, dateFilter],
  );

  const grouped = useMemo(() => groupReservationsByDate(filtered), [filtered]);
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          ...
          />
          <label className="flex shrink-0 flex-col gap-1.5 sm:min-w-[200px]">
            <span className="text-[10px] tracking-[0.15em] text-[#8a7a7e] uppercase">
              Filtrar por fecha
            </span>
            <div className="flex gap-2">
              <input
                ...
                className="glass-panel min-h-[44px] w-full rounded-xl border border-white/[0.08] bg-white/[0.03] px-3 text-sm text-[#f7efe8] outline-none focus:border-[#e8b4bc]/40"
              />
              {dateFilter ? (
                <button
                  type="button"
                  onClick={() => setDateFilter("")}
                  className="min-h-[44px] shrink-0 rounded-xl px-3 text-[10px] tracking-wide text-[#a89a9e] uppercase ring-1 ring-white/[0.08] hover:text-[#f7efe8]"
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-8">
          {grouped.map((group) => (
            <section key={group.date} className="space-y-3">
              <header className="sticky top-0 z-10 border-b border-[#e8b4bc]/15 bg-[#120a0e]/90 py-2 backdrop-blur-md">
                <h2 className="font-serif text-xl capitalize text-[#f7efe8]">
                  {formatGroupDateHeader(group.date)}
                </h2>
                <p className="text-xs text-[#8a7a7e]">
                  ...
                  >
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      <div className="min-w-0 flex-1 space-y-1">
                        ...
                        <p className="text-sm text-[#c4b4b8]">{formatTime(r.booking_time)}</p>
                        <p className="text-xs text-[#6a5c60]">
                          {r.email} · {r.phone}
                        </p>
                        {r.message ? (
                          <p className="mt-2 text-sm italic text-[#8a7a7e]">
